...
?>
          ...
          <table class="table"><tr><td>File<td>PMT<td>ATC<td>RUSH
<?php
foreach ($hashes as $file => $hashBySite) {
    if (count($hashBySite)==3 && count(array_unique($hashBySite))==1)
...
# find bad phrases with: git --git-dir /var/www/virtuals/acls.net/wp-content/themes/pmtsystem/.git grep --name-only -e 'aclsrenewal.org' -e 'acls.net' remotes/origin/rush
?>
        </table>
        </div> <!-- /container -->
